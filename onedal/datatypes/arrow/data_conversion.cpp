...
#ifdef ONEDAL_DATA_PARALLEL
        // if located on a SYCL device, use the queue. Generate queue from the ArrowDeviceArray struct.
        // If a queue is given, it will override the general queue that would be generated
        // from get_queue_by_device_id. This is important as arrow oneAPI devices are not aware of
        // sub-devices, priority queues or sycl contexts, which can cause the data only to be usable
